...
object Test1 {
  def main(args: Array[String]): Unit = {
    ...
    val max_hot_temp = df_agg.select("COUNTRY_FULL","max_temp_avg").orderBy(col("max_temp_avg").desc).first()
    println(max_hot_temp)
    val max_cold_temp = df_agg.select("COUNTRY_FULL","min_temp_avg").orderBy(col("min_temp_avg")).first()
    println(max_cold_temp)
    ...
    val highest_con_days = streak.select("COUNTRY_FULL","streak_1").orderBy(col("streak_1").desc)first()
    println(highest_con_days)
